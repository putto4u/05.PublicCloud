# 실무 환경 디렉토리 생성 및 일일 작업(Daily Workflow) 파이프라인 가이드
...
로컬 PC의 도구 세팅이 완료되었다면, 터미널을 열고 아래의 디렉토리 생성 절차를 진행합니다.
...
mkdir -p global-microservices-project
cd global-microservices-project

# 2. 인프라 모듈(순수 테라폼) 및 Live(테라그런트) 디렉토리 생성
mkdir -p infrastructure/modules/aws-prerequisites
mkdir -p infrastructure/live/000-prerequisites/ap-northeast-2
...
생성된 디렉토리에 맞게 앞서 설계했던 사전 작업(Prerequisites)의 전체 코드를 아래에 통합하여 제공합니다. VS Code 등의 에디터를 사용하여 지정된 경로에 정확히 파일을 생성하고 코드를 작성하십시오.
...
remote_state {
  ...
  config = {
    ...
    bucket         = "global-microservices-tfstate-apne2-12345" # 본인만의 고유한 이름으로 변경 필수
...
* **생성 위치:** `infrastructure/modules/aws-prerequisites/main.tf`
...
resource "aws_ecr_repository" "backend_repo" {
  name                 = "globalapp-backend" # 저장소 이름 설정
...
resource "aws_iam_role" "eks_cluster_role" {
  name = "globalapp-eks-cluster-role"

# 이 역할(Role)을 누구(EKS 서비스)가 사용할 수 있는지 정의하는 신뢰 정책
  assume_role_policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      Action = "sts:AssumeRole"   # 역할을 맡을 수 있도록 허용하는 동작
      Effect = "Allow"
      Principal = {
...
# AmazonEKSClusterPolicy: 클러스터 운영에 필요한 표준 권한을 포함한 AWS 관리형 정책
  policy_arn = "arn:aws:iam::aws:policy/AmazonEKSClusterPolicy"
  role       = aws_iam_role.eks_cluster_role.name # 위에서 만든 역할과 연결
}
...
* **생성 위치:** `infrastructure/live/000-prerequisites/ap-northeast-2/terragrunt.hcl`
...
terraform {
  source = "../../../../modules/aws-prerequisites"
}

# 모듈에 필요한 변수를 주입합니다.
inputs = {
  region      = "ap-northeast-2"
  environment = "global-prereq"
}
...
```bash
# 1. 실행할 대상 디렉토리로 이동
cd infrastructure/live/000-prerequisites/ap-northeast-2
...
git add .
git commit -m "feat: 000-prerequisites 모듈 및 terragrunt 설정 완료"
