...
오늘의 목표는 온프레미스 대역(**10.10.0.0/16**)을 기준으로 AWS 글로벌 인프라 대역을 **10.20.0.0/16(한국)**, **10.30.0.0/16(미국)**, **10.40.0.0/16(유럽)**으로 각각 할당하고, 데이터베이스 이전을 대비한 보안 구조를 한국 리전 스포크에 먼저 반영하는 것입니다.
...
| **유럽 리전 (euw2)** | **eu-west-2** | **10.40.0.0/16** | **(신규)** 향후 확장 예정인 영국 런던 스포크 |
...
유럽 리전 확장을 고려하여 `vpc_cidr`의 기본값을 유연하게 관리하며, 온프레미스 대역을 명시합니다.
...
# 하위 서브넷들은 vpc_cidr에 맞춰 Terragrunt inputs에서 주입받는 것을 권장합니다.
...
DB 이전을 고려한 보안 그룹(`db_migration_sg`)이 포함된 표준 VPC 로직입니다.
...
```bash
# 1. 자원 폐기 (NAT Gateway 등 고정 비용 자원 삭제)
...
git add .
git commit -m "feat: 글로벌 3개 대륙(10.20~10.40) 설계 및 유럽 리전 계획 반영"
...
**Next Step: 10.20 대역 기반 EKS 클러스터 구성 및 IRSA 설정**
